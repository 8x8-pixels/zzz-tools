...
Usage:
    python disc_capture_v2.py --monitor [--out result_live] [--debug] [--stdout] [--tesseract PATH]
...
class LiveDiscScanner:
    def __init__(self, args):
        ...
        self.stdout_mode = getattr(args, 'stdout', False)
        ...
        if self.stable_frames >= 2 and not self.captured_current:
            ...
            if pytesseract and not self.args.no_ocr:
                ...
                if len(parsed_data.get("sub_stats", [])) > 0:
                    ...
                    if self.stdout_mode:
                        emit_disc(self._transform_disc_for_output(parsed_data))
...
def main():
    ...
    parser.add_argument("--stdout", action="store_true", help="Emit each disc as JSON line to stdout")
