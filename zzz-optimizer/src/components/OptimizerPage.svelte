<script lang="ts">
  ...
  import SupportCharPanel from './SupportCharPanel.svelte';
  import type {
    CharacterDef,
    Disc,
    DiscSetDef,
  ...
  async function handleFiles(event: Event) {
    ...
    for (const file of files) {
      try {
        const discs = await loadDiscFile(file);
        if (discs.length > 0) {
          const setName = discs[0].name;
    ...
    <aside class="config-panel">
      ...
      <section>
        ...
        <label class="file-btn">
          ファイル選択
          <input type="file" accept=".json" multiple on:change={handleFiles} class="hidden" />
        </label>
